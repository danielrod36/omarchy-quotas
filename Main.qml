...
Item {
  ...
  readonly property string usageDir: (Quickshell.env("XDG_STATE_HOME") || home + "/.local/state") + "/omarchy/quotas/usage"


  // The update orchestrator ships beside this file inside the plugin.
  readonly property string updateScript: {
    var url = String(Qt.resolvedUrl("usage-update"))
  ...
  Process {
    ...
    stderr: StdioCollector {
      waitForEnd: true
      onStreamFinished: if (text.trim() !== "") console.warn("quotas", text.trim())
    }
  }

  function updateCommand(kind, agentIds) {
    var command = [root.updateScript]
  ...
  // Providers without usable credentials: the panel's config page walks
  // these — hidden from the overview, one setup card each.
  readonly property var setupProviders: {
    ...
    for (var i = 0; i < agents.length; i++) {
      var record = agents[i] ? agents[i].record : null
      if (!record || !record.id) continue
      if (record.configured !== false) continue
      if (!providerEnabled(String(record.id))) continue
      var display = displayProvider(record)
      if (display.setupGuide.length > 0) result.push(display)
    }
    return result
  }
  ...
  // A provider this machine carries credentials for earns its tab even while
  // its endpoint is down, so the error card stays readable.
  ...
  Process {
    ...
    stderr: StdioCollector {
      waitForEnd: true
      onStreamFinished: if (text.trim() !== "") console.warn("quotas/sync", text.trim())
    }
  }
  ...
  function parseSyncScanOutput(output) {
    ...
    function flush() {
      ...
      try {
        ...
        console.warn("quotas/sync", "Ignoring bad snapshot", currentPath, e)
      }
      currentPath = ""
      currentJson = []
    }
